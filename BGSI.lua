...
-- Functions
    ...
    function AutoPickupAll()
        ...
        print("[AUTOPICKUP] Initializing continuous pickup loop...")

        while true do
            if _G.AutoPickupAll and Player.Character then
                    ...
                    for _, child in ipairs(Rendered:GetChildren()) do
                        ...
                        if (child:IsA("Folder") or child:IsA("Model")) and 
                           (name:find("collectibles") or name:find("drops") or name:find("pickups")) then
                            ...
                            print("[AUTOPICKUP DEBUG] Found Chunker by name: " .. child.Name)
                            break
                        end
                    end

                    -- 2. FALLBACK SEARCH: If name search fails, use the descendant count method
                    if not CollectiblesChunker then
                        local maxDescendants = 0
                        for _, child in ipairs(Rendered:GetChildren()) do
                            if (child:IsA("Folder") or child:IsA("Model")) then
                                local descendantCount = #child:GetDescendants()
                                -- Assume the folder with many descendants and more than 10 children is the correct one.
                        ...
                        if CollectiblesChunker then
                            print("[AUTOPICKUP DEBUG] Found Chunker by descendant count: " .. CollectiblesChunker.Name)
                        end
                    end
                end
                ...
                if CollectiblesChunker then
                    ...
                    if collectedCount > 0 then
                        ...
                        print("[AUTOPICKUP DEBUG] Found Chunker, but no valid items inside.")
                    end
                else
                    print("[AUTOPICKUP ERROR] Could not locate Collectibles Chunker. Game structure may have changed.")
                end

                task.wait(0.1) 
            else
                -- Wait longer when the toggle is off to reduce script load
                task.wait(0.5)
            end
        end
    end
...
-- Farming Tab
    ...
    FarmingTab:AddToggle({
        ...
        Callback = function(Value)
            ...
            -- The AutoPickupAll function runs in a continuous loop started at the bottom, 
            -- and checks this global toggle on every iteration.
