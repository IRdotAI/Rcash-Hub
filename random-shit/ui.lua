local StatusContainer = {}  -- Stores {ScriptKey = {Currency = "...", Status = "...", Title = "..."}, ...}
local ActiveKey = nil       -- Key of the status currently displayed
local ActiveIndex = 0       -- Index for cycling through statuses
local __currentUI = nil     -- The main ScreenGui object
...
local CURRENCY_EMOJIS = {
    ...
    ["Secret"] = "🐶",    -- Webhook script theme
    ["Potion"] = "🧪",    -- Potion script theme
    ["Default"] = "✨"
}

local UIManager = {}

-- Function to handle UI display updates and cycling
local function updateDisplay()
    local keys = {}
    for k in pairs(StatusContainer) do 
        if k ~= "__Loader" then -- Exclude the initial loader status from cycling
            table.insert(keys, k) 
        end
    end
    -- If only the loader status is present, insert it so something displays
    ...
    ActiveIndex = (ActiveIndex % #keys) + 1
    ...
    -- Access properties directly via FindFirstChild for maximum safety
...
-- Function to initialize the UI elements (Called only after environment is ready)
function UIManager.Initialize()
    local Players = game:GetService("Players")
    local LocalPlayer = Players.LocalPlayer
    
    -- We assume this function is only called when LocalPlayer is valid and PlayerGui exists
...
--- Public function for ALL scripts to call
...
-- ==========================================================
-- SAFELY START THE UI MANAGER AFTER PLAYERGUI IS CONFIRMED READY
-- ==========================================================
local function startUp()
    ...
    -- Wait up to 10 seconds for PlayerGui to be created (this is the fix)
...
-- Defer execution to allow the game engine to complete initial setup
task.spawn(startUp)
